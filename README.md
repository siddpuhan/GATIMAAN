...
## 🏛️ Phase 3: Final Information Architecture & Workspace Roles

GATIMAAN enforces a clear, role-aware operational model across all endpoints and UI surfaces:

### 1. Persona Navigation & Access Model
- **CUSTOMER:**
  - `→ Citizen Portal` (Discover services, generate digital token, track live queue, view wait times).
  - No access to administrative or operator controls.
- **OPERATOR:**
  - `OPERATE → Queue Desk` (`/admin/queue`): The single unified operational workspace to open counter shifts, call next tickets, serve citizens, complete sessions, and record no-shows.
  - Automatic redirect away from administrative configuration, insight, or system routes.
- **ADMIN (Admin Command Centre):**
  - **OPERATE:**
    - `Overview` (`/admin`): Real-time Center Command telemetry (active desks, services, footfall, demand levels, live predictions).
    - `Queue Desk` (`/admin/queue`): Operational desk.
  - **CONFIGURE:**
    - `Services` (`/admin/services`): Department & service catalog management.
    - `Counters & Desks` (`/admin/counters`): Physical desk registry and shift configuration.
  - **INSIGHT:**
    - `Footfall` (`/admin/footfall`): Real-time IoT gate monitoring & hourly footfall trends.
    - `Analytics` (`/admin/analytics`): Operational SLAs, wait times, throughput analytics.
    - `Predictions` (`/admin/predictions`): Statistical queue ETA & demand surge projections.
  - **SYSTEM:**
    - `Notifications` (`/admin/notifications`): Dispatch logs and citizen alert queues.
    - `Settings` (`/admin/settings`): Centre operating hours and system configurations.

### 2. Operational Workspace Rule
`Queue Desk` (`/admin/queue`) is the **sole, canonical operational workspace**. All legacy `Queue Operations` duplicates have been removed to guarantee clean state and prevent double-calling race conditions.
